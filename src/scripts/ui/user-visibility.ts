...
class UserVisibilityPTR2e {
  ...
  static process(html: HTMLElement, options: ProcessOptions = {}): void {
    ...
    const document = (options.document as Actor.ConfiguredInstance | ChatMessage.ConfiguredInstance) ?? message?.actor ?? message ?? null;
    if (document) {
  ...
  static processMessageSender(message: ChatMessage.ConfiguredInstance, html: HTMLElement): void {
    // Hide the sender name from the card if it can't be seen from the canvas
    const token =
      message.token ?? (message.actor ? new CONFIG.Token.documentClass(message.actor.prototypeToken.toObject()) : null);
    if (token) {
...
interface ProcessOptions {
  ...
  message?: ChatMessage.ConfiguredInstance | null;
}
