 ...
 */
export function getInitialSkillList(): foundry.data.fields.SchemaField.PersistedType<SkillSchema>[] {
...
export function partialSkillToSkill(partialSkill: Partial<foundry.data.fields.SchemaField.PersistedType<SkillSchema>>): foundry.data.fields.SchemaField.PersistedType<SkillSchema> {
  const schema = SkillPTR2e.schema;
  const initial = (schema.initial as () => foundry.data.fields.SchemaField.PersistedType<SkillSchema>)();

  if (!partialSkill.slug) throw new Error("Partial Skill is missing slug");
  return foundry.utils.mergeObject(initial, partialSkill) as foundry.data.fields.SchemaField.PersistedType<SkillSchema>;
}
