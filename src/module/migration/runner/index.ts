...
export class MigrationRunner extends MigrationRunnerBase {
  ...
  static async ensureSchemaVersion(document: Actor.ConfiguredInstance | Item.ConfiguredInstance, migrations: MigrationBase[]): Promise<void> {
    ...
    if ("items" in document && "prototypeToken" in document) {
      for (const item of document.items as unknown as Item.ConfiguredInstance[]) {
  ...
  async #migrateDocuments<TDocument extends Actor.ConfiguredInstance | Item.ConfiguredInstance>(
    collection: WorldCollection<TDocument, "Actor" | "Item"> | CompendiumCollection<TDocument>,
    ...
    for (const document of collection.contents) {
      if (updateGroup.length === 100) {
        try {
          // @ts-expect-error - FIXME: Type error? Investigate later.
  ...
  async #migrateItem(migrations: MigrationBase[], item: Item.ConfiguredInstance): Promise<Item.ConstructorData | null> {
  ...
  async #migrateActor(
  ...
  ): Promise<Actor.ConstructorData | null> {
    ...
    const baseItems = [...baseActor.items] as Item.ConfiguredInstance['_source'][];
    const updatedItems = [...updatedActor.items as unknown as Item.ConstructorData[]] as Item.ConfiguredInstance['_source'][];
    ...
    const finalUpdated = itemDiff.updated.filter((i) => actor.items.has(i._id as string));
  ...
  async #migrateSceneToken(
    ...
    migrations: MigrationBase[],
  ): Promise<TokenDocument.ConstructorData | null> {
  ...
  async runCompendiumMigration<T extends Actor.ConfiguredInstance | Item.ConfiguredInstance>(
  ...
  async runMigrations(migrations: MigrationBase[]): Promise<void> {
    ...
    for (const scene of game.scenes) {
      for (const token of scene.tokens as unknown as TokenDocument.ConfiguredInstance[]) {
        const { actor } = token;
        if (!actor) continue;
