...
export class GlobalPerkGeneratorConfig extends foundry.applications.api.HandlebarsApplicationMixin(ApplicationV2Expanded) {
  static override DEFAULT_OPTIONS = fu.mergeObject(
    super.DEFAULT_OPTIONS,
    {
      ...
      actions: {
        "open-entry": function (this: GlobalPerkGeneratorConfig, _event, element) {
        ...
        "export-config": async function (this: GlobalPerkGeneratorConfig, _event, element) {
          const id = element.dataset.id;
          if (!id) return void ui.notifications.error("The config could not be found");
        ...
        "delete-config": async function (this: GlobalPerkGeneratorConfig, _event, element) {
          const id = element.dataset.id;
          if (!id) return void ui.notifications.error("The config could not be found");
