...
> {
  ...
  override validate(value: AssignmentType, options?: foundry.data.fields.DataField.ValidationOptions<foundry.data.fields.DataField<Options, AssignmentType, InitializedType, PersistedType>>): void | foundry.data.validation.DataModelValidationFailure {
  ...
  override _validateModel(changes: AnyObject) {
    (this.model as unknown as typeof foundry.abstract.DataModel<foundry.data.fields.DataSchema, foundry.abstract.DataModel.Any>).validateJoint(changes as never); // FVTT Type guards this with type never, which is sound, but we'll ignore it this time ;)
  }
}
