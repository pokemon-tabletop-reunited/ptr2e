...
class FormActiveEffectSystem extends ActiveEffectSystem<FormActiveEffectSchema> {
  ...
  override async _preCreate(data: foundry.abstract.TypeDataModel.ParentAssignmentType<FormActiveEffectSchema, ActiveEffect.ConfiguredInstance>, options: foundry.abstract.Document.PreCreateOptions<any>, user: User.ConfiguredInstance): Promise<boolean | void> {
    const self = this as FormActiveEffectSystem;
  ...
  protected override _preUpdate(
    this: FormActiveEffectSystem,
    changes: DeepPartial<foundry.abstract.TypeDataModel.ParentAssignmentType<FormActiveEffectSchema, ActiveEffect.ConfiguredInstance>>,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const self = this as FormActiveEffectSystem;
    // Make sure the toggle roll option is dealt with
    const trigger = (changes?.system?.trigger ?? self.trigger) as "manual" | "automatic";
    if (trigger === "manual") {
      const change = self.changes.find(change => change.key === "manual-forme-toggle");
      if (!change) {
        const change = {
        ...
        // @ts-expect-error - Correct type
        changes.system.changes.unshift(change);
      }
    }
    else {
      changes.system ??= {};
      changes.system.changes ??= [];
      // @ts-expect-error - Correct type
      changes.system.changes = (changes.system.changes as this['_source']['changes']).filter(change => change.key !== "manual-forme-toggle");
    }
    ...
    const { removedConditions, removedAdditionalProperties } = (() => {
      ...
      if (Array.isArray(changes?.system?.conditions)) {
        for (const condition of self.conditions) {
          if (typeof condition === "string" && !changes.system.conditions.includes(condition)) {
            removedConditions.push(condition);
          }
          else if (!changes.system.conditions.some(c => typeof c === "string" ? false : R.isDeepEqual(c, condition))) {
            removedConditions.push(condition);
          }
        }
      }
      const removedAdditionalProperties = [...removedConditions];
      if (changes?.system?.trigger && changes.system.trigger !== self.trigger) {
    ...
    if (changes?.system?.changes?.length) {
      for (const change of changes.system.changes as PTR.ActiveEffect.Changes.Source[]) {
        ...
        const conditions = (() => {
          if (change.key === "manual-forme-toggle") return changes?.system?.conditions ?? self.conditions;

          if (self.trigger === "manual" || changes.system.trigger === "manual") {
            return [...((changes?.system?.conditions ?? self.conditions) as this['conditions']), self.manualOption];
          }

          return changes?.system?.conditions ?? self.conditions;
    ...
    return super._preUpdate(changes, options, userId);
  }
}

export default FormActiveEffectSystem;
export type { FormActiveEffectSystem }