import { ActorPTR2e, DeferredEphemeralEffect, DeferredValueParams } from "@actor";
    ...
    return async (params: DeferredValueParams = {}): Promise<EffectSourcePTR2e[] | null> => {
      ...
      try {
        for (const alteration of this.alterations ?? []) {
          alteration.applyTo(source as ItemSourcePTR2e, (params.injectables?.actor ?? params.resolvables?.actor) as ActorPTR2e | undefined);
        }
      } catch (error) {
