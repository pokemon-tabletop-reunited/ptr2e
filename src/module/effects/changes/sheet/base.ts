...
import type { RuleValue } from "@module/effects/data.ts";
...
class ChangeForm<TChange extends ChangeModel = ChangeModel> {
  ...
  async updateItem(updates: Partial<TChange["_source"]> | Record<string, unknown>): Promise<void> {
    ...
    changes[this.index] = result as PTR.ActiveEffect.Changes.Instance;
...
function cleanDataUsingSchema(schema: Record<string, foundry.data.fields.DataField>, data: Record<string, unknown>): void {
  ...
  for (const [key, field] of Object.entries(schema)) {
    ...
    if (field instanceof ResolvableValueField) {
      data[key] = field.clean(data[key as keyof typeof data] as RuleValue);
...
interface ChangeFormContext<TChange extends PTR.ActiveEffect.Changes.Instance>
...
interface ChangeFormOptions<TChange extends PTR.ActiveEffect.Changes.Instance = PTR.ActiveEffect.Changes.Instance> {
