...
class CombatTrackerPTR2e extends CombatTracker {
  ...
  override async getData(options?: Partial<ApplicationOptions>): Promise<object> {
    ...
    data.turns = data.turns.map((turn: CombatTurn) => {
    ...
    if (preview) {
      ...
      data.turns = data.turns.sort((a: CombatTurn, b: CombatTurn) => {
    ...
    const delaySummons = data.turns.filter((c: CombatTurn) => c.delay !== undefined);
