...
import { sluggify, tagify } from "@utils";
...
class ActorSheetPTR2e extends ActorSheet<ActorPTR2e> {
    ...
    override activateListeners($html: JQuery<HTMLElement>) {
        ...
        for (const taggifyElement of $html.find(".ptr-tagify")) {
            tagify(taggifyElement as HTMLInputElement, {
                traits: $(taggifyElement).hasClass("system-traits"),
            });
        }

        $html.find(".item-controls .item-to-chat").on("click", async (event) => {
            const uuid = (event.currentTarget.closest(".item-controls") as HTMLElement)?.dataset
                ?.uuid;
            if (!uuid) return;
        ...
        $html.find(".item-controls .item-edit").on("click", async (event) => {
            const { uuid, itemId, effectId } = (
                event.currentTarget.closest(".item-controls") as HTMLElement
            )?.dataset;
            if (uuid) {
                const document = await fromUuid(uuid);
        ...
        $html.find(".attack .rollable").on("click", async (event) => {
            const slug = (event.currentTarget.closest("li.attack[data-action]") as HTMLElement)
                ?.dataset?.action;
            if (!slug) return;
            const parentUuid = (
                event.currentTarget.closest("ul.action-list-attack[data-parent]") as HTMLElement
            )?.dataset?.parent;
        ...
        $html
            .find(".stats-chart, .link-to-stats h2")
            .on("dblclick", () => new StatsForm({ document: this.actor }).render(true));
