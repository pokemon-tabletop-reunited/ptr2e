...
export default function HasEmbed<BaseClass extends TemplateConstructor>(baseClass: BaseClass, baseType: string) {
  abstract class TemplateClass extends baseClass {
    override async toEmbed(_config: TextEditor.DocumentHTMLEmbedConfig, options: TextEditor.EnrichmentOptions & {_embedDepth?: number}, additionalProperties: Record<string, unknown> = {}, type = baseType): Promise<HTMLElement | HTMLCollection | null> {
      options = { ...options, _embedDepth: (options._embedDepth ?? 0) + 1, /*TODO: Investigate why this is here: `relativeTo: this` */};
