    ...
    static override defineSchema() {
        ...
        return {
            target: new fields.StringField({
                required: true, choices: Object.values(PTRCONSTS.TargetOptions).reduce<Record<string,string>>((acc, target) => ({...acc, [target]: target}), {}), initial: PTRCONSTS.TargetOptions.CREATURE, label: "PTR2E.FIELDS.target.label", hint: "PTR2E.FIELDS.target.hint"
            }),
            distance: new fields.NumberField({ required: true, initial: 10, min: 0, integer: true, label: "PTR2E.FIELDS.distance.label", hint: "PTR2E.FIELDS.distance.hint"}),
