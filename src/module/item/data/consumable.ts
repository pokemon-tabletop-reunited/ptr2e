...
import { HasActions, HasContainer, HasDescription, HasEmbed, HasGearData, HasMigrations, HasPublication, HasSlug, HasTraits, Trait } from "@module/data/index.ts";
...
const ConsumableExtension = HasEmbed(HasMigrations(HasTraits(HasDescription(HasSlug(HasContainer(HasPublication(HasGearData(HasActions(foundry.abstract.TypeDataModel)))))))), "consumable");
...
type ConsumableSystemSchemaExtension = SlugSchema & MigrationSchema & TraitsSchema & DescriptionSchema & ContainerSchema & GearSchema & PublicationSchema & ActionsSchema
