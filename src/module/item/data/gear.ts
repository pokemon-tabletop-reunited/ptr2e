...
export default abstract class GearSystem extends HasEmbed(HasIdentification(HasGearData(HasBase(foundry.abstract.TypeDataModel))), "gear")<GearSystemSchema, Item.ConfiguredInstance> {
  ...
  override async _preCreate(data: foundry.abstract.TypeDataModel.ParentAssignmentType<GearSystemSchema, Item.ConfiguredInstance>, options: foundry.abstract.Document.PreCreateOptions<any>, user: User): Promise<boolean | void> {
