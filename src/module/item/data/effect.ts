...
export default abstract class EffectSystem extends HasEmbed(HasTraits(HasMigrations(HasDescription(HasSlug(foundry.abstract.TypeDataModel)))), "effect-item")<EffectSchema, Item.ConfiguredInstance> {
  ...
  override async _preCreate(data: foundry.abstract.TypeDataModel.ParentAssignmentType<EffectSchema, Item.ConfiguredInstance>, options: foundry.abstract.Document.PreCreateOptions<any>, user: User.ConfiguredInstance): Promise<boolean | void> {
