  ...
  findSpecies(config: ImageSpeciesResolverConfig) {
    ...
    function formMatch(result: string) {
      ...
      for (const form of forms) {
        if (result.includes(form) || form === "_Token") {
          count++;
        }
      }
      return {
    ...
    if (shiny) {
      const shinyNeedle = config.female ? [...baseNeedle, 'f', 's'] : [...baseNeedle, 's'];
    ...
    if (config.female) formsNeedleSuffix.unshift('f');
    ...
    // form matched partially, make sure that the partial match isn't female if other forms are present
    if (config.female) {
      formsNeedleSuffix.shift();
